        ...
        brandsLink.addEventListener('click', function (e) {
            ...
            productGroup.forEach(item => {
                if (!item.classList.contains(filterId)) {
                    sliderNumber = item.closest('.slick-track');
                    sliderIndex = item.closest('.slick-slide').slickIndex;
            ...
            const time = sliderNumber * sliderIndex;
            console.log(time);
            document.querySelector('.slick-track').style.transform = `translate3d(${time}px, 0px, 0px)`;
            // 
        })
    }
})